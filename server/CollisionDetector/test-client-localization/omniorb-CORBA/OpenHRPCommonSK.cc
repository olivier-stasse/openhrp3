...
void
OpenHRP::Distance::operator>>= (cdrStream &_n) const
{
  (const LinkPair&) pair >>= _n;
  minD >>= _n;
...
void
OpenHRP::Distance::operator<<= (cdrStream &_n)
{
  (LinkPair&)pair <<= _n;
  (::CORBA::Double&)minD <<= _n;
  _n.unmarshalArrayDouble((_CORBA_Double*)(point0), 3);
  _n.unmarshalArrayDouble((_CORBA_Double*)(point1), 3);
